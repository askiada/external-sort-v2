...
func sort(ctx context.Context, inputS3URL, outputS3URL string, log model.Logger) error {
	...
	chunkRdrFn := func(step string, idx int) (io.Reader, error) {
		chunkFilename := path.Join(
			filepath.Dir(outputKey),
			fmt.Sprintf("chunks/%s_%s_%d%s", baseOutputNoExt, step, idx, filepath.Ext(outputKey)),
		)
	...
	tsvKeyFn := func(row interface{}) (model.Key, error) {
		tKey, err := key.AllocateCsv(row, 1, 2, 11, 12, 13)
		if err != nil {
			return nil, err
		}
	...
	inputBucket, inputKey, err := s3handler.Parse(inputS3URL)
	if err != nil {
		return fmt.Errorf("can't parse input s3 url: %w", err)
	}

	_, err = s3Cli.HeadObject(ctx, inputBucket, inputKey)
	if err != nil {
		return fmt.Errorf("can't get header: %w", err)
	}
